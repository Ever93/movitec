...
;# NEEDS WORKcheck to see what the image file looks like and can we
		# replace the full image file with the relative file?
        ...
        if {![file exists $full_file] && !$vTcl(borrow)} {
			...
			append msg " 'No' aborts cut/copy operation."
            ...
            if {$response == yes} {
				...
				set full_file [vTcl:relTo $source_file $vTcl(borrow_dir)]
				set vTcl(abort) 0
            } else {
				set vTcl(abort) 1
				return
