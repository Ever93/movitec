...
proc vTcl:get_subopts_special {opts w} {
    ...
    if {[info exists ::widgets::${w}::subOptions::save]} {
        ...
        ## to sister function to do
        ## so. ::widgets::${w}::subOptions::save is set in
        ## ::menu_edit::new_item.
...
#         }
	...
	set new_menu_name ""	
    if {$vTcl(copy) && $vTcl(copy_class) eq "Menu"} {
        # Removed "&& $len == 3" from above if clause and added the
        # following "if".
        if {$len > 3} {
        }
    ...
    } elseif {$vTcl(copy) && $vTcl(copy_class) eq "Menubutton" && $len == 3} {
        # new      ;# NEEDS WORK menubutton
        append result "\$\{target\}"
		set new_menu_name [vTcl:new_widget_name Mnu ""]       
    } else {
    ...
    if {$p != ""} {            ;#set exists [info exists vTcl(pr,font_dft)]
	    ...
	    append result \
			" \\\n[vTcl:new_clean_pairs $p $target \${base}$new_menu_name]\n"
		}	
    ...
    if {$vTcl(copy) && $vTcl(copy_class) eq "Menubutton" && $len == 3} {
        $target configure -menu $target.m
    }
    switch $class {
        Listbox -
        Text {
...
append result "### SPOT dump_widget_opt A\n"
...
proc vTcl:dump_popup_menu {} {
    # This is from the days when popup menus where the children of the
    # root window. That is no longer the case, so we may not need this
    # function,
    global vTcl
    set output ""
...
proc vTcl:dump:widgets {target} {
	...
	foreach i $tree {
		...
		if {$class eq "Menu"} {
			if {[string first .mnu $i] > -1 ||
				[string first .men $i] > -1} {
				continue
			}
		}
...
proc vTcl:dump:gather_menu_info {} {
    ...
    foreach child $children {
        ...
        for {set index 0} {$index <= $entries} {incr index} {
            ...
            catch {set image [$child entrycget $index -selectaaaaaaaaaaaimage]}
            if {$image != ""} {
                lappend vTcl(dump,userImages) $image
            }			
        }
