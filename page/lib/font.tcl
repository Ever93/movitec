...
proc vTcl:font:prompt_user_font {target option} {
    ...
    if {$r == ""} {
        ...
        $target configure $option $r
        vTcl:create_handles $target  ;# Added 6/15/19
        # refresh property manager
        vTcl:update_widget_info $target
        vTcl:prop:update_attr
    }
...
proc vTcl:font:prompt_user_font_2 {font_desc} {
    ...
    # ::ChooseFont::ChooseFont is in choosefont.tcl
    global vTcl
    if {$font_desc == ""} {
    ...
    # If the user canceled in the font chooser then r is "".
    if {$r != ""} {
        set r [font actual $r]
        set vTcl(att_changed) 1
    } else {
        set r $font_desc
    }
...
proc vTcl:font:init_stock {} {
    ...
    set gd [is_default_font gui_font_dft default]
    set vTcl(font,gui_font_dft) $gd
    set gt [is_default_font gui_font_text text]
    set vTcl(font,gui_font_text) $gt
    set gf [is_default_font gui_font_fixed fixed]
    set vTcl(font,gui_font_fixed) $gf
    set gm [is_default_font gui_font_menu menu]
    set vTcl(font,gui_font_menu) $gm
    set gtip [is_default_font gui_font_tooltip tooltip]
    set vTcl(font,gui_font_tooltip) $gtip
    set gtree [is_default_font gui_font_treeview treeview]
    set vTcl(font,gui_font_treeview) $gtree
    set vTcl(default_gui_font_names) [list $gd $gt $gf $gm $gtip $gtree]
    ...
    set vTcl(actual_gui_font_tooltip_name) $gtip
    set vTcl(actual_gui_font_treeview_name) $gtree
    # These are descriptions like "-family times -size 12 -weight bold".
    set vTcl(actual_gui_font_dft_desc) \
        [vTcl:condense_font_description $vTcl(pr,gui_font_dft)]
    set vTcl(actual_gui_font_text_desc) \
        [vTcl:condense_font_description $vTcl(pr,gui_font_text)]
    set vTcl(actual_gui_font_fixed_desc) \
        [vTcl:condense_font_description $vTcl(pr,gui_font_fixed)]
    set vTcl(actual_gui_font_menu_desc) \
        [vTcl:condense_font_description $vTcl(pr,gui_font_menu)]
    set vTcl(actual_gui_font_tooltip_desc) \
        [vTcl:condense_font_description $vTcl(pr,gui_font_tooltip)]
    set vTcl(actual_gui_font_treeview_desc) \
        [vTcl:condense_font_description $vTcl(pr,gui_font_treeview)]
}

proc is_default_font {font {name {}}} {
    # Check to see if the default font is one of the tk default fonts.
    global vTcl
    if { [string index $vTcl(pr,$font) 0] != "T"} {
        set gd [vTcl:font:add_font $vTcl(pr,$font) stock $name]
    } else {
        set gd $vTcl(pr,$font)
    }
    return $gd
...
proc {vTcl:font:add_font} {font_descr font_type {newkey {}} {check_fonts {1}}} {
    ...
    # creating a font. So if you want to assign a name to a font then
...
#             set font_c [vTcl:condense_font_description  $f]
#             # puts $fileID \"[font configure $font_c]\"
#             puts $fileID \"$font_c\"
#         }
#     }
    foreach f {dft text fixed menu tooltip treeview} {
        # if {[string first "Tk" $vTcl(actual_gui_font_${f}_desc)] == -1} {}
        if {$vTcl(actual_gui_font_${f}_name) in $vTcl(standard_fonts)} {
            puts $fileID \
                "set vTcl(actual_gui_font_${f}_desc) \
                        $vTcl(actual_gui_font_${f}_desc)"
            puts $fileID \
                "set vTcl(actual_gui_font_${f}_name) \
                        $vTcl(actual_gui_font_${f}_name)"
        } else {
            puts $fileID \
                "set desc \"[vTcl:condense_font_description \
                       $vTcl(actual_gui_font_${f}_desc)]\""
            puts $fileID \
                "set vTcl(actual_gui_font_${f}_desc) \$desc"
            puts $fileID \
                "set vTcl(actual_gui_font_${f}_name) \[font create {*}\$desc\]"
        }
    }
}
...
# This stuff caused me fits. Finally, got it working by commenting out
# the NoEncaseOption below AND returning the value from the
# translation WITHOUT the embracing quotes. 6/26/19

TranslateOption    -font vTcl:font:translate
NoEncaseOption     -font 1
NoEncaseOptionWhen -font vTcl:font:noencase_when

proc vTcl:font:translate {value} {

    # Confusion here: sometimes we may want to have the quotes and
    # sometimes not.
    if [info exists ::vTcl(fonts,$value,font_descr)] {
        set value \"$::vTcl(fonts,$value,font_descr)\"   ;# Original
        #set value  $::vTcl(fonts,$value,font_descr)
    }
    return $value
}

proc vTcl:font:noencase_when {value} {
    # Special cases when we want to skip encasing the option
    # value. Added by Rozen

    # Original version had:
    # if [string match {\[*\]} $i] { set noencase 1 }
    # Don't see why it was necessary.
    if {[string match *font* $value]} {
        # I think this means it is a font name rather than a
        # description. Names are words like "font14" or
        # "TkDefaultFont".
        return 1
    }
    if {[string match {"*"} $value] ||
        [string match "\{*\}" $value]} {
        # It is already encased.
        return 1
    }
    return 0
}
