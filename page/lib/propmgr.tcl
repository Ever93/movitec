...
#         }
#     }
#     $f configure -width [winfo width $vTcl(gui,ae)]  ;# NEEDS WORK variable width
#     # if {$on == ""} {
...
proc vTclWindow.vTcl.ae {args} {
    ...
    set base $top
    if {[winfo exists $ae]} {
        # The width may have changed so I
        # (1) want to remove the three windows in f1,f2, and f3 so that
        # they can be replaced with the new size,
        set c  $vTcl(gui,ae,canvas)
        set f1 $c.f1                              ; # Widget Info Frame
        set f2 $c.f2                              ; # Widget Attribute Frame
        set f3 $c.f3                              ; # Widget Geometry Frame
        set ae $vTcl(gui,ae)
        $c delete intro attr geom
        # (2) set "geom_ae" to the new size,
        #dmsg BINGO width =  [winfo width $ae]
        set width [winfo width $ae]
        set geom_ae [wm geometry $ae]
        set w $f1.f
        #set geom_ae [wm geometry $ae [wm geometry $ae]]
        # (3) skip the creation of the AE top level.
        set create_ae 0
        # wm deiconify $ae
        # vTcl:prop:recalc_canvas
        # return
    } else {
        set geom [vTcl:split_geom_m $vTcl(geometry,.vTcl.ae)]
        set sw [lindex $geom 0]
        if {[llength $geom] < 4} {
            set geom_ae "300x400-0+0"
        } elseif {$sw < 200} {
            set geom_ae "300x400-0+0"
        } else {
...
if {$create_ae} {
    toplevel $ae -class vTc
    wm withdraw $ae ;#$vTcl(gui,ae)
...
if {$create_ae} {
    label $f1.l -text "Widget"     -relief raised -bg #aaaaaa -bd 1  \
        -anchor center  -fg black
    pack $f1.l -side top -fill x

    label $f2.l -text "Attributes" -relief raised -bg #aaaaaa -bd 1 \
        -anchor center  -fg black
    pack $f2.l -side top -fill x

    label $f3.l -text "Geometry"   -relief raised -bg #aaaaaa -bd 1 \
        -anchor center  -fg black
    pack $f3.l -side top -fill x

    # bind $f1.l <ButtonPress> {vTcl:prop:set_visible info}
    # bind $f2.l <ButtonPress> {vTcl:prop:set_visible attr}
    # bind $f3.l <ButtonPress> {vTcl:prop:set_visible geomgeometry}
    ...
    label $w.ln -text "Widget" -width 13 -anchor w
    ...
    frame $w.filler1 -width 20
    label $w.lc -text "Class"  -width 13 -anchor w
    vTcl:entry $w.ec  -textvariable vTcl(w,class) \
        -relief sunken \
        -state disabled -disabledbackground $vTcl(pr,disablebg) \
        -disabledforeground $vTcl(pr,disablefg)
    frame $w.filler2 -width 20
    label $w.lm -text "Manager" -width 13 -anchor w
    vTcl:entry $w.em -textvariable vTcl(w,manager) \
        -relief sunken \
        -state disabled -disabledbackground $vTcl(pr,disablebg) \
        -disabledforeground $vTcl(pr,disablefg)
    frame $w.filler3 -width 20
    label $w.la -text "Alias"  -width 13 -anchor w
    # ea is the entry field where an alias is entered.
    vTcl:entry $w.ea -textvariable vTcl(w,alias) \
        -disabledbackground $vTcl(pr,disablebg) \
        -disabledforeground $vTcl(pr,disablefg) \
        -relief sunken -foreground #000000       ;# NEEDS WORK dark
    label $w.li -text "Insert Point" -width 13 -anchor w
    frame $w.filler4 -width 20
    vTcl:entry $w.ei -textvariable vTcl(w,insert) \
        -relief sunken \
        -state normal -bg $vTcl(pr,disablebg) -fg  $vTcl(pr,disablefg)
    frame $w.filler5 -width 20
    ...
    grid $w.ln $w.en $w.filler1 -padx 0 -pady 1 -sticky news
    grid $w.lc $w.ec $w.filler2 -padx 0 -pady 1 -sticky news
    grid $w.lm $w.em $w.filler3 -padx 0 -pady 1 -sticky news
    grid $w.la $w.ea $w.filler4 -padx 0 -pady 1 -sticky news
    if {[info exists vTcl(show_variant)] && $vTcl(show_variant)} {
        grid $w.lv $w.ev $w.filler6 -padx 0 -pady 1 -sticky news
    }
    grid $w.li $w.ei $w.filler5 -padx 0 -pady 1 -sticky news
...
set vTcl(ae_width) [winfo width $ae]
# dpr vTcl(ae_width)
#     bind $ae <Configure>    {
#         if {$vTcl(ae_width) != [winfo width %W]} {
#             puts "%W size: [winfo width %W]x[winfo height %W]"
#         vTclWindow.vTcl.ae
#         }
...
proc vTcl:configure_ae_event {} {
    ...
    global vTcl
    dmsg
    set g [winfo geometry .vTcl.ae]
    dpr g
    dpr vTcl(ae_width)
    # dmsg width:: [winfo width .vTcl.ae]
    # catch {
    # if {[winfo width .vTcl.ae] != $vTcl(ae_width)} {
    #     puts BINGO
    #     vTclWindow.vTcl.ae
    # }
    # }
...
proc vTcl:prop:new_attr {top option variable config_cmd \
    ...
    switch $type {
        ...
        image {
            ...
            vTcl:entry ${base}.l -relief sunken  \
                -textvariable $variable -width 8 \
                -highlightthickness 1 -fg black  -insertborderwidth 3 \
                -state normal -disabledbackground $vTcl(pr,disablebg) \
                -disabledforeground $vTcl(pr,disablefg) \
				-background orange
            button ${base}.f \
                -image ellipses  -width 12 \
    ...
    # filler is a frame put in to occupy space to be used for vertical
    # scroll bar so that the ends of the real data will not be over
    # ridden by the scrollbar.
    set filler [frame ${base}_filler -width 20]
    if {[vTcl:streq $prefix "opt"]} {
        set saveCheck [checkbutton ${base}_save -pady 0]
        vTcl:set_balloon $saveCheck "Check to save option"
        grid $top.$option $base $saveCheck $filler -sticky news
    } else {
        grid $top.$option $base $filler -sticky news
        global tcl_platform
