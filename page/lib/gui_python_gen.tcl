...
foreach window $vTcl(tops) {
    ...
    if {[string first ".pop" $window] == 0} { continue }
    # For debugging only
...
proc vTcl:python_generate_context_menu {root_list} {
   ...
   "$vTcl(tab)$vTcl(tab)self.$widname = tk.Menu(self.top, tearoff=0)\n"
        append vTcl(popup_code) \
            "$vTcl(tab)$vTcl(tab)self.$widname.configure(background=_bgcolor)\n"
        append vTcl(popup_code) \
            "$vTcl(tab)$vTcl(tab)self.$widname.configure(foreground=_fgcolor)\n"
        append vTcl(popup_code) \
          "$vTcl(tab)$vTcl(tab)self.$widname.configure(activebackground=_ana2color)\n"
        if {[::colorDlg::dark_color $vTcl(analog_color_m)]} {
            append vTcl(popup_code) \
            "$vTcl(tab)$vTcl(tab)self.$widname.configure(activeforeground='white')\n"
        } else {
            append vTcl(popup_code) \
				"$vTcl(tab)$vTcl(tab)self.$widname.configure(activeforeground='black')\n"
        }
        set font_cfg [$popup cget -font]
        set font_cfg [vTcl:condense_font_description $font_cfg]
        append vTcl(popup_code) \
            "$vTcl(tab)$vTcl(tab)self.$widname.configure(font=\"$font_cfg\")\n"
        # Now to process the menu.  This code generates the actual
        # menu entries in vTcl(popup_code) and are added below.
        set vTcl(in_popup) 1
        vTcl:python_process_menu $popup $widname $widname vTcl(popup_code)
        append vTcl(popup_code) \
    ...
    # Want to remove all "self." clauses except for "self.top".
    # if {$vTcl(in_popup)} {
    #     regsub -all {self\.top} $vTcl(popup_code) "__zzzz" vTcl(popup_code)
    #     regsub -all {self[.]} $vTcl(popup_code) "" vTcl(popup_code)
    #     regsub -all {__zzzz} $vTcl(popup_code) "self.top" vTcl(popup_code)
    # }
    set vTcl(in_popup) 0
...
#stop
    if {![info exists vTcl(py_initfunc)]} { set vTcl(py_initfunc) "" }
    append source \
        "
class $classname:
...
proc vTcl:py_tk_variable_code {} {
	...
	set vTcl(p_v_list) [lsort -unique -index 0 $vTcl(p_v_list)]
	foreach sublist $vTcl(p_v_list) {
		foreach {var class trash} $sublist { }
        append vTcl(tkvar_code) \
            "$vTcl(tab)$vTcl(tab)$var = " tk.${class}() "\n"
    }
...
$vTcl(tab)$vTcl(tab)AutoScroll.__init__(self, master)"
    ...
    if {[lsearch $vTcl(helper_list) Scrolledframe] > -1} {
        ;# NEEDS WORK Scrolledframe
        append ttk_helper \
"

class Scrolledframe(AutoScroll, tk.Frame):
$vTcl(tab)'''A standard Tkinter Frame widget with scrollbars that will
$vTcl(tab)automatically show/hide as needed.'''
$vTcl(tab)@_create_container
$vTcl(tab)def __init__(self, master, **kw):
$vTcl(tab)$vTcl(tab)tk.Frame.__init__(self, master, **kw)
$vTcl(tab)$vTcl(tab)AutoScroll.__init__(self, master)"
    }
...
proc vTcl:py_source_frame {window prefix parentframe} {
    ...
    set cchildren [winfo children $window]
    foreach widget [winfo children $window] { # Rozen
        ...
        # }
        if {[string first "#" $widget] > -1} continue
...
#                    append vTcl(py_initfunc) \
        ...
        Scrolledframe {
            ...
            append vTcl(py_initfunc) [vTcl:emit_style_code]
            append vTcl(py_initfunc) \
                    "$vTcl(tab2)self.$widname = Scrolledframe($pframe)\n"
            vTcl:relative_placement $parent
            vTcl:python_configure_widget $target.01 $widname $cmdname $widclass
            vTcl:inner_frame $widname
            #vTcl:python_load_cmd $target $widname
            ...
            set vTcl(add_helper_code) 1
            lappend vTcl(helper_list) Scrolledframe
        }
        TProgressbar {
            set config [$target configure]
            append vTcl(py_initfunc) [vTcl:emit_style_code]
...
proc vTcl:python_menu_options { opts {index {}} {menu_name {}} \
    ...
    foreach op $opts {
        ...
        switch -exact -- $o {
			...
			-selectimage { ;# This option handled as an entryconfig. }
            -image {
                #append option_string \
            ...
            -variable {
                ...
                #    # Variable in imported module
                    ...
                    if {[lsearch -exact $vTcl(g_v_list) $v] == -1} {
                        ...
                        #lappend vTcl(g_v_list) $l
						if {$vTcl(in_popup)} {
                            if {[lsearch -exact $vTcl(p_v_list) $v] == -1}  {
								# vTcl(p_v_list) is a list of lists so
								# that I can use lsort to remove
								# duplicates. See
								# vTcl:py_tk_variable_code.
								lappend vTcl(p_v_list) [list self.$v $m $target]
                           }
...
proc vTcl:python_create_image {target index {option "-image"}} {
    ...
    set image  [$target entrycget $index $option] 
    if {$image == ""} return ""
