...
namespace eval ::vTcl::compounds {
    ...
    proc createCompound {target type compoundName \
      ...
      "namespace eval \{::vTcl::compounds::${type}::[list $compoundName]\} \{\n"
		...
		# This follows a call chain which is very confusing but does
		# not search for select images. I am unable to see how to
		# reliably include the search for selectimages. Therefore, I
		# am replacing it with simpler code.
        #set imagesDef [defineCompoundImages $target]
        #append output $imagesDef
		set all_images [vTcl::widgets::look_for_all_images $target]
		append output $all_images
        append output "\n\n"
        ...
        append output \
            "$vTcl(tab)regexp \{^\\.bor\\d+\} \$target top_part\n"
        ...
        if {$class == "Toplevel"} {
           ...
           append output "    st items \[split \$target .\]\n"
        ...
        set vTcl(copy_width)    [winfo width $w_widget]
		    ...
		    # Writes out the clipboard file so that it can be modified
		    # and read back in. A debugging tool.

		    # set tail [file tail $vTcl(project,file)]
		    # set rootname [file rootname $vTcl(project,file)]
		    # set fn [file join "." $rootname.cpy]
		    # set chan [open $fn w]
		    # puts $chan $output
		    # close $chan
        return $output
...
#"
    ...
    proc autoPlaceCompound {type compoundName gmgr gopt} {
        ...
        } elseif {$rootclass == "Popupmenu"} {
            set namePrefix "m"
            # set target $vTcl(w,insert)
            set target [vTcl:new_widget_name Popupmenu $vTcl(w,insert)]
        } else {
            set namePrefix cpd
...
proc insertCompound {target type compoundName {gmgr pack} {gopt ""}} {
	...
	;# NEEDS WORK Want to use this stuff when I'ved debugged the copy stuff
    ...
    if {$vTcl(copy_class) ni  "Menu Popupmenu" } {
        if {$gmgr != "wm"} {
