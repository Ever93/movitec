...
proc vTcl:widget:lib:lib_core {args} {
    # Loads the top section of the Widget Toolbar. It is called from
    # vTcl:setup_gui in ../page.tcl.
    global vTcl
...
proc vTcl:wm:conf_dflt_origin {target var value} {
    global vTcl
    vTcl:change
    set ::widgets::${target}::dflt,origin  $vTcl(w,wm,dflt,origin) ;#$value
}
...
proc vTcl:edit_target_menu {target} {
    # We come here when we click to start editing the menu bar.
    global vTcl
    ...
    if {$menu == ""} {
        ...
        menu $menu -bg $vTcl(actual_gui_menu_bg) -tearoff 0
    ...
    # Now we go edit it by opening the menu editor .located in menus.tcl.
...
proc vTcl:core:menutranslate {value} {
    ...
    if [regexp {((\.\w+)+)} $value matchAll path] {
        if {$matchAll == $value} {
...
proc vTcl:core:scrolltranslate {value} {
    ...
    if [regexp {((\.\w+)+) set} $value matchAll path] {
                set path [vTcl:base_name $path]
        return "\"$path set\""
    }
        return $value
...
proc vTcl:core:noencasewhen {value} {
    # Attempts to find special case when we do not want to encase an
    # option. Added the last string test 6/26/19.
...
# edit sites in a paned window, etc. The edit stuff also relies on a
# number of routines in the ::vTcl::ui::attributes namespace which is
# defined at the last section misc.tcl.
...
proc vTclWindow.vTcl.itemEdit {base} {
    ...
    # Up button
    set image [vTcl:light_or_dark_image page_up]    ;# NEEDS WORK dark
    vTcl:toolbar_button $base.fra34.but37 \
        -image $image -command "::vTcl::itemEdit::moveUpOrDown $base up"
        # -image up -command "::vTcl::itemEdit::moveUpOrDown $base up"
    vTcl:set_balloon $base.fra34.but37 {Move Up}
    # Down button
    set image [vTcl:light_or_dark_image page_down]    ;# NEEDS WORK dark
    vTcl:toolbar_button $base.fra34.but38 \
        -image $image -command "::vTcl::itemEdit::moveUpOrDown $base down"
    ...
    # Rozen the list box where the pages are listed.  This is the left
    # window. Also called ItemsListbox.
...
# Because I didn't know where better to put this function. It is part
# of the edit tab stuff.
...
namespace eval ::vTcl::itemEdit {
    ...
    proc edit {target cmds} {
        ...
        variable class
        global vTcl
        incr counter         ;# NEEDS WORK - do I want this counter stuff???
        set top .vTcl.itemEdit_$counter
        set vTcl(edit_window) $top
        set suffix($top) $counter

        # Handles the case where we are invoking the editor on a notebook tab.
        set target_class [winfo class $target]
        ...
        if {$target_class eq "TLabelframe"} {
            set parent [winfo parent $target]
            set p_class [winfo class $parent]
...
# }
    ...
    proc initProperties {top} {
        ...
        foreach option $options {
            ...
            set config_cmd "
               ...
               "
            # ui::newAttribute below is located in misc.tcl and mimics
            # vTcl:prop:new_attr in propmgr.tcl in the way it fills in
            # attribute frames.
    ...
    proc selectItem {top index} {
        ...
        foreach option $allOptions($top) {
            ...
            # the ::ui::attribute stuff is in misc.tcl
